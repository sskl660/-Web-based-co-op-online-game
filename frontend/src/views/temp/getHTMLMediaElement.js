...
function getHTMLMediaElement(mediaElement, config) {
  ...
  if (buttons.has('mute-audio')) {
    ...
    muteAudio.className =
      'control ' + (config.toggle.has('mute-audio') ? 'unmute-audio selected' : 'mute-audio');
  ...
  if (buttons.has('mute-video')) {
    ...
    muteVideo.className =
      'control ' + (config.toggle.has('mute-video') ? 'unmute-video selected' : 'mute-video');
  ...
  if (buttons.has('record-audio')) {
    ...
    recordAudio.className =
      'control ' +
      (config.toggle.has('record-audio') ? 'stop-recording-audio selected' : 'record-audio');
  ...
  if (buttons.has('record-video')) {
    ...
    recordVideo.className =
      'control ' +
      (config.toggle.has('record-video') ? 'stop-recording-video selected' : 'record-video');
  ...
  if (buttons.has('full-screen')) {
    ...
    function screenStateChange(e) {
      ...
      var isFullScreeMode =
        document.webkitIsFullScreen || document.mozFullScreen || document.fullscreen;
  ...
  function adjustControls() {
    ...
    if (slider) {
      ...
      volumeControl.style.marginLeft =
        mediaElementContainer.clientWidth - volumeControl.clientWidth - 2 + 'px';
    }

    volumeControl.style.marginTop =
      mediaElementContainer.clientHeight - volumeControl.clientHeight - 2 + 'px';
...
function getAudioElement(mediaElement, config) {
  ...
  function adjustControls() {
    ...
    mediaControls.style.marginTop =
      mediaElementContainer.clientHeight - mediaControls.clientHeight - 6 + 'px';
    if (times < 10) {
      times++;
      setTimeout(adjustControls, 1000);
    } else times = 0;
