            ...
            <button v-bind:id="'btn-'+index" class="btn" @click="showTeam(index)">{{index}}</button>
                ...
                <div class="team-group">
                    <div v-for="index in totalTeam" :key="index" class="line">
                        <div v-bind:class="'team-group team-group-' + index" v-show="room.teamline[index]" @click="changeTeam(index)">
...
export default {
    ...
    data: function() {
        return {
            openmodal: false, //게임 선택 모달 플레그
            ssafymind_explain: false, //설명용 플레그
            ...
            if (this.room.gameType == 1) {
                ...
                axios({
                    ...
                    data: {
                        roomId: this.room.id,
                        host: this.room.host,
                        exist: this.room.teamline,//열린 팀 정보
                        gameType: this.room.gameType,
                    },
                })
                    .then(() => {
